...
const ReviewCard = ({ avatarSrc, name, email, reviewText }) => (
    <div className="w-full p-6 bg-[#a6bb8d]/80 rounded-md overflow-hidden mb-6 flex flex-col">
        <div className="flex items-center mb-4">
            <div className="w-[50px] h-[50px] rounded-full overflow-hidden mr-4">
                <img src={avatarSrc || PlaceholderImage} alt={`${name} Avatar`} className="w-full h-full object-cover"/>
            </div>
            <div>
...
export const Detail = () => {
    ...
    useEffect(() => {
        axiosClient.get(`/podcasts/${podcastId}`)
                ...
                setSelectedPodcast(data); // asumsi pakai Resource
                })
                .catch((error) => {
    ...
    useEffect(() => {
        axiosClient.get(`/podcasts/${podcastId}/related`)
            .then(({ data }) => {
            setRelatedPodcasts(data.data); // asumsi pakai Laravel resource
            })
            .catch(error => {
            console.error('Gagal mengambil podcast terkait:', error);
            });
    }, [podcastId]);
     ...
     useEffect(() => {
        ...
        .then(({ data }) => {
            setUser(data);
            console.log("User login:", data);//perhatikan console
        })
        .catch(error => {
            console.error("Gagal mengambil data user:", error);//console
        });
    }, []);
    ...
    console.log("selectedPodcast1:", selectedPodcast);

    if (selectedPodcast == null) {
        return (
        <div className="min-h-screen bg-[#eae7b1] p-4 md:p-8 flex flex-col items-center justify-center pt-24">
            <p className="text-2xl text-[#3c6255] mb-4">Sedang memuat detail podcast...</p>
        </div>
        );
    }
            ...
            <div className="max-w-screen-lg mx-auto bg-[#eae7b1] rounded-lg shadow-xl p-6 md:p-8">
                ...
                <section className="flex flex-col md:flex-row items-center md:items-start gap-8 mb-12">
                    <div className="relative w-full md:w-2/5 flex justify-center items-center">
                        <img src={selectedPodcast.image_url} alt={`${selectedPodcast.title} Background`} className="w-full h-auto max-h-[350px] object-cover rounded-lg opacity-50"/>
                        <img src={selectedPodcast.image_url} alt={`${selectedPodcast.title} Cover`} className="absolute inset-0 m-auto w-3/5 md:w-3/4 max-w-[173px] h-auto object-cover rounded-lg shadow-lg"/>
                    </div>
                        ...
                        {relatedPodcasts.length > 0 ? (
                            relatedPodcasts.map((related, index) => (
                                <RelatedPodcastCard
                                    ...
                                    coverSrc={`/storage/podcast/${related.image_url}`}
                                    rating={related.average_rating}
                ...
                <section className="mb-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div className="bg-[#eae7b1] rounded-lg p-4 shadow-md">
                            ...
                            <p className="text-base font-semibold text-left text-[#3c6255] mb-4">{selectedPodcast.channel?.name || ""}</p>

                            <p className="text-xs text-left text-[#3c6255] mb-1">Creator</p>
                            <p className="text-base font-semibold text-left text-[#3c6255] mb-4">{selectedPodcast.creator_name}</p>

                            <p className="text-xs text-left text-[#3c6255] mb-1">Show-Web</p>
                            <p className="text-base font-semibold text-left text-[#3c6255]">{selectedPodcast.show_web}</p>
                        </div>

                        <div className="bg-[#eae7b1] rounded-lg p-4 shadow-md">
                            <p className="text-xs text-left text-[#3c6255] mb-1">Years Active</p>
                            <p className="text-base font-semibold text-left text-[#3c6255] mb-4">{`${selectedPodcast.start_year} - ${selectedPodcast.end_year || 'Sekarang'}`}</p>

                            <p className="text-xs text-left text-[#3c6255] mb-1">Episodes</p>
                            <p className="text-base font-semibold text-left text-[#3c6255] mb-4">{selectedPodcast.episodes?.length || 0}</p>

                            <p className="text-xs text-left text-[#3c6255] mb-1">Genre</p>
                            <p className="text-base font-semibold text-left text-[#3c6255]">{selectedPodcast.genre}</p>
                        </div>

                        <div className="bg-[#eae7b1] rounded-lg p-4 shadow-md">
                            <p className="text-xs text-left text-[#3c6255] mb-1">License</p>
                            <p className="text-base font-semibold text-left text-[#3c6255] mb-4">{selectedPodcast.license}</p>

                            <p className="text-xs text-left text-[#3c6255] mb-1">Copyright</p>
                            <p className="text-base font-semibold text-left text-[#3c6255] mb-4">{selectedPodcast.copyright}</p>
                        </div>
                    </div>
                </section>
