...
const PlaylistCard = ({ playlist }) => (
  <Link to={`/playlistviewall/${playlist.id}`} className="block bg-[#3c6255] p-2 w-auto h-auto rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
    ...
    <div className="flex flex-wrap gap-2 mb-4 justify-center">
      {playlist.podcasts.slice(0, 4).map((episode, index) => (
        <img
          key={index}
...
export const Profil = () => {
  ...
  const playlistsToDisplay = playlists.slice(0, 4);
  const reviewedPodcastsToDisplay = reviewedPodcasts.slice(0, 3);
 ...
 useEffect(() => {
    axiosClient.get('/user')
      ...
      axiosClient.get('/reviewed')
        .then(({ data }) => setReviewedPodcasts(data.data));
      ...
      <div className="max-w-screen-xl mx-auto px-4">
        <div className='flex justify-between items-start mb-10'>
          ...
          />
            ...
            <button onClick={onLogout}className='bg-[#3c6255] text-[#eae7b1] px-4 py-2 rounded-md hover:opacity-90'>
              Log out
            </button>
